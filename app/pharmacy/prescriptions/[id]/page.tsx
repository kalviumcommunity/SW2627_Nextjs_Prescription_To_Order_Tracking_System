  ...
  if (error && !prescription) {
    return (
      <div className="space-y-6">
        <ErrorState title="Unable to load prescription" message={error} onRetry={loadPrescription} />
      </div>
    );
  }

  if (!prescription) {
    return <EmptyState title="Prescription not found" description="The requested prescription is no longer available." />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-200 pb-5">
        <div>
          <Link href="/pharmacy/prescriptions" className="text-sm font-semibold text-blue-700 hover:underline">Back to prescription queue</Link>
          <h1 className="text-2xl font-bold text-gray-900 mt-3">Prescription #{prescription.id.slice(-8).toUpperCase()}</h1>
          <p className="text-sm text-gray-500 mt-1">Created {formatDate(prescription.createdAt)}</p>
        </div>
        <div>
      ...
      {prescription.status === 'PENDING' && (
        <Card className="border-blue-200 bg-blue-50/40">
          <CardHeader><CardTitle className="text-blue-950">Prescription Fulfillment Actions</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-700">Verify the prescription items and submit the terminal fulfillment outcome. A successful fill registers this pharmacy and creates the fulfillment record.</p>
            <div>
              <label className="block text-xs font-semibold uppercase text-gray-600 mb-1">Fulfillment Notes (Optional)</label>
              <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Optional fulfillment notes" className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900" disabled={fulfilling} />
            </div>
            <div className="flex flex-wrap gap-3">
              <Button onClick={() => setConfirmAction('FILLED')} variant="primary">Dispense &amp; Mark as Filled</Button>
              <Button onClick={() => setConfirmAction('CANNOT_FILL')} variant="destructive">Cannot Fill Prescription</Button>
            </div>
          </CardContent>
        </Card>
      )}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            <div><p className="text-xs uppercase font-semibold text-gray-500">Name</p><p className="font-semibold text-gray-900">{prescription.patient.name}</p></div>
            <div className="grid grid-cols-2 gap-4"><div><p className="text-xs uppercase font-semibold text-gray-500">Age</p><p className="text-gray-700">{prescription.patient.age}</p></div><div><p className="text-xs uppercase font-semibold text-gray-500">Gender</p><p className="text-gray-700">{prescription.patient.gender}</p></div></div>
            <div><p className="text-xs uppercase font-semibold text-gray-500">Contact</p><p className="text-gray-700 break-words">{prescription.patient.contactInfo}</p></div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <p className="text-gray-500">{prescription.doctor.specialization}</p>
            <p className="font-mono text-gray-700">{prescription.doctor.licenseNumber}</p>
            <p className="text-gray-700 break-all">{prescription.doctor.email}</p>
          </CardContent>
        </Card>
      </div>
      <Card>
        <CardHeader><CardTitle>Medicines and directions</CardTitle></CardHeader>
        <CardContent className="p-0">
          <div className="divide-y divide-gray-100">
            {prescription.medicines.map((item) => (
              <div key={item.id} className="p-5 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4">
                <div><h2 className="font-semibold text-gray-900">{item.medicine.name}</h2><p className="text-sm text-gray-500">{item.medicine.genericName}</p></div>
                <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                  <div><dt className="text-xs uppercase font-semibold text-gray-500">Dosage</dt><dd className="text-gray-700 mt-1">{item.dosage}</dd></div>
                  <div><dt className="text-xs uppercase font-semibold text-gray-500">Frequency</dt><dd className="text-gray-700 mt-1">{item.frequency}</dd></div>
                  <div><dt className="text-xs uppercase font-semibold text-gray-500">Duration</dt><dd className="text-gray-700 mt-1">{item.duration}</dd></div>
                </dl>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card><CardHeader><CardTitle>Fulfillment timeline</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><p>Created: {formatDate(prescription.createdAt)}</p><p>Fulfilled: {formatDate(prescription.filledAt || prescription.fill?.filledAt || null)}</p>{prescription.fill?.pharmacy && <p>Dispensed by: {prescription.fill.pharmacy.pharmacyName}</p>}{prescription.fill?.notes && <p>Notes: {prescription.fill.notes}</p>}</CardContent></Card>
      </div>
