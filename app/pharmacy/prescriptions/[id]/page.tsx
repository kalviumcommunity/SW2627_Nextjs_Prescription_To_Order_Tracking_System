...
export default function PharmacyPrescriptionDetailPage({ params }: { params: { id: string } }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card><CardHeader><CardTitle>Prescription document</CardTitle></CardHeader><CardContent className="text-sm">{prescription.documentAvailable && prescription.documentRef ? <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"><p className="break-all text-gray-700">Reference: <span className="font-mono">{prescription.documentRef}</span></p><a href={`/api/prescriptions/${prescription.id}/document`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-md bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100 transition-colors border border-blue-200 shrink-0"><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>View Document</a></div> : <p className="text-gray-500">No prescription document is available.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Fulfillment timeline</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><p>Created: {formatDate(prescription.createdAt)}</p><p>Fulfilled: {formatDate(prescription.filledAt || prescription.fill?.filledAt || null)}</p>{prescription.fill?.pharmacy && <p>Dispensed by: {prescription.fill.pharmacy.pharmacyName}</p>}{prescription.fill?.notes && <p>Notes: {prescription.fill.notes}</p>}</CardContent></Card></div>
